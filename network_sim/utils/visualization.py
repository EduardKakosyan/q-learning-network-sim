...
def save_network_visualization(
    ...
    """
    if not simulator.graph:
        raise ValueError("Simulator graph is empty.")
    ...
    edge_labels = {
        (u, v): f"{graph[u][v]['delay']*1000:.1f}ms" for u, v in graph.edges()
    }
...
def plot_metrics(
    ...
    """
    if not metrics_list:
        raise ValueError("Metrics list is empty. Provide a non-empty list of metrics.")

    router_types = [metrics.get("router_type", "Unknown") for metrics in metrics_list]

    fig, axes = plt.subplots(1, 3, figsize=(12, 5))

    throughputs = [m.get("throughput", 0) for m in metrics_list]
    delays = [m.get("average_delay", 0) for m in metrics_list]
    loss_rates = [m.get("packet_loss_rate", 0) for m in metrics_list]
...
def plot_link_utilizations(
    ...
    for i, metrics in enumerate(metrics_list):
        ...
        ax.set_title(
            f"Link Utilization (Router: {metrics.get('router_type', 'Unknown')})"
        )
...
def plot_packet_journeys(
    ...
    Args:
        simulator_list: List of NetworkSimulator instances.
    ...
    if not simulator_list:
        raise ValueError("Simulator list is empty.")

    num_simulators = len(simulator_list)
    fig, axes = plt.subplots(
        num_simulators, 1, figsize=(12, 5 * num_simulators), sharex=True
    )
    ...
    for i, simulator in enumerate(simulator_list):
        ...
        if packet_ids is None:
            ...
            if len(completed_packets) > max_packets:
                sorted_packets = sorted(
                    completed_packets, key=lambda p: p.creation_time
                )
        ...
        ax.set_title(
            f"Packet Journey Through Network (Router: {simulator.router_type})"
        )
...
def plot_buffer_usages(
    ...
    for i, simulator in enumerate(simulator_list):
        ...
        ax.set_title(f"Buffer Usage Over Time (Router: {simulator.router_type})")
